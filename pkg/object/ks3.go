...
func (s *ks3) Get(key string, off, limit int64, getters ...AttrGetter) (io.ReadCloser, error) {
	...
	if resp != nil {
		...
		attrs.SetRequestID(aws2.ToString(resp.Metadata[s3RequestIDKey]))
		attrs.SetStorageClass(aws2.ToString(resp.Metadata[s3StorageClassHdr]))
	}
	if err != nil {
		return nil, err
	}
...
func (s *ks3) Put(key string, in io.Reader, getters ...AttrGetter) error {
	...
	if resp != nil {
		attrs := applyGetters(getters...)
		attrs.SetRequestID(aws2.ToString(resp.Metadata[s3RequestIDKey])).SetStorageClass(s.sc)
	}
	return err
}
